    ...
    public class SearchQueriesService : ISearchQueries
    {
        ...
        public List<TicketPrintableModel> SearchTicket(string stringToSearch, int idRol, int userId)
        {
            ...
            using (_database)
            {
                ...
                {
                    var foundedTicketbyId = filteredTickets.Where(currentTicket => currentTicket.idLocal.Equals(toSearchInt));
                    List<TicketPrintableModel> founderdList = foundedTicketbyId.ToList();
                    if (founderdList.Any())
                    {
                        return founderdList;
                    }
        ...
        /// <summary>
        /// Find the ticket that has the same idLocal assigned to it. Does not respect ticket ownership or technician assignment.
        /// </summary>
